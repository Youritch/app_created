...
async function openDirApp(event) {
	const { canceled, filePaths } = await dialog.showOpenDialog({ properties: ["openDirectory"] /*filters: [{ name: "html", extensions: ["html"] }] */ });
	...
	for (const f in files) {
		if (!(await api.file.existFile(`${filePaths[0]}/${files[f]}`))) {
			if (files[f].includes(".")) {
				...
				mainWin.webContents.send("popup", { type: "warn", title: files[f] + " à été crer", message: filePaths[0] });
			} else {
	...
	if (!(await api.file.existFile(`data/customApp/${info.name}.json`))) {
		...
		console.log();
		if (!complet) return mainWin.webContents.send("popup", { type: "error", title: 'Remplir "<strong>info.json</strong>"', message: 'il faut remplir le fichier "<strong>info.json</strong>" qui se trouve ' + filePaths[0] + "\nIl manque les champs : " + manque.join(", ") });
		api.file.writeFile(`data/customApp/${info.name}.json`, { info: info, path: filePaths[0] });
	}
	...
	customAppWin.loadFile();
	mainWin.webContents.send("popup", { type: "info", title: "Tout est bon !", message: "Toute l'app a été crer/mis à jours." });
	return filePaths[0];
}
async function handleFileComp() {
	const { canceled, filePaths } = await dialog.showOpenDialog({ properties: ["openFile"], filters: [{ name: "html", extensions: ["html"] }] });
	if (!canceled) {
		//const htmlfile = require(path.relative(__dirname,filePaths[0]));
	...
	const menu = Menu.buildFromTemplate([
		{
			...
			submenu: [
				...
				{
					click: () => {
						win.webContents.send("popup", { type: "info", title: "Tous se passe bien !", message: "et oui !" });
					},
...
app.whenReady().then(() => {
	...
	ipcMain.on("load", async (event) => {
		...
		try {
			...
			event.sender.send("load-page", [data]);
		} catch (err) {
			console.log(err);
		}
	});
